...
\usepackage{vaucanson-g} %Automata
...
sublanguage defined, computes the co-faulty and and co-non-faulty languages of
an adjacent module.
...
\begin{algorithmic}[1]
	...
	\EndFor
	\Procedure{co-languages}{$i$, $F_i$, $N_i$}
		\ForAll{$j \in I \mid j \textrm{ is adjacent to } i$}
			\State $F'_j \leftarrow (
			P_{\Sigma_j \rightarrow \Sigma_j \cap \Sigma_i}^{-1}(C_j) 
			\cap F_i^{-1})\cap C_j$
			\State $N'_j \leftarrow (C_j^{-1} \cap N_i^{-1})\cap
			C_j$ \State $K \leftarrow F'_j \cap N'_j$ \State $F'_j \leftarrow F'_j \backslash K$
			...
			\If{$F'_j \backslash F_j \neq \emptyset \lor 
				...
				\State \textproc{co-languages}$(j, F_j, N_j)$
			\EndIf
		\EndFor
	\EndProcedure
	
	\State $F_i \leftarrow 
		P_{\Sigma_i \rightarrow \bigcap_{j\neq i}^I \Sigma_j}(L_{i,f})$
	\State $N_i \leftarrow 
		P_{\Sigma_i \rightarrow \bigcap_{j\neq i}^I \Sigma_j}(L_{i,nf})$
	\State \textproc{co-languages}($i$, $F_i$, $N_i$)
	
	\ForAll{$j \in I$}
		\State $L_{j,cf}:= L_j \cap P_{\Sigma_j \rightarrow \Sigma_c}^{-1}(F_j)$
		\State $L_{j,cnf}:= L_j \cap P_{\Sigma_j \rightarrow \Sigma_c}^{-1}(N_j)$
	\EndFor
	\\
	\Return $\{L_{j,cf}\}$, $\{L_{j,cnf}\}$
\end{algorithmic}
...
\begin{algorithmic}[1]
	...
	\ForAll{$j\in I \mid j\neq i$}
		...
		\If{$F_{j} = \emptyset$}
			\If{$N_{j} = \emptyset \lor L_{j,cf} \textrm{ has a cycle}$}
			\State report that the system is not diagnosable 
			\EndIf
		\EndIf
	\EndFor
	\ForAll{$j, k \in I \mid j, k\neq i$}
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[->,>=stealth, node distance=2.5cm, auto, shorten >=1pt,
...
;
\end{tikzpicture}
\caption{Automaton, marking language $L_1$. $\Sigma_o = \{o_1\}$, $\Sigma_f =
\{f\}$}
\label{fig:G1}
\end{figure}
...
;
\end{tikzpicture}
\caption{Automata, marking languages $L_2, L_3$ and $L_4$}
\label{fig:G2_4}
\end{figure}
...
\begin{figure}[t]
\centering
\begin{tikzpicture}[->,>=stealth, node distance=2.1cm, auto, shorten >=1pt,
semithick, initial text=,	
	every node/.style={scale=0.7},
    every state/.style={fill=green,text=black},
    accepting/.style={double distance=1.5pt, outer sep=0.75pt+\pgflinewidth}
...
;
\end{tikzpicture}
\caption{Automata, marking $F_1$ and $N_1$ languages}
\label{fig:FN1}
\end{figure}


% Calculating co-faulty language F2
\begin{figure}[t]
\centering
\begin{tikzpicture}[->,>=stealth, node distance=2.1cm, auto, shorten >=1pt,
semithick, initial text=,	
...
;
\end{tikzpicture}
\caption{Automata, marking $F_2$ and $N_2$ languages}
\label{fig:FN2}
\end{figure}



% Calculating co-faulty language F4
\begin{figure}[t]
\centering
\begin{tikzpicture}[->,>=stealth, node distance=2.1cm, auto, shorten >=1pt,
semithick, initial text=,	
...
\end{tikzpicture}
\caption{Automata, marking $F_4$ and $N_4$ sublanguages}
\label{fig:FN4}
\end{figure}
