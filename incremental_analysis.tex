...
\usepackage{natbib}            % you should have natbib.sty for IFAC
...
\author[First]{Dmitry Myadzelets} 
\author[Second]{Andrea Paoli}

\thanks[footnoteinfo]{This work was supported by the Erasmus Mundus Action 2
programme of the European Union}

\address[First]{Center for Research on Complex Automated Systems (CASY), DEI,
University of Bologna, Viale Pepoli 3/2, 40123, Bologna, Italy (e-mail:
dmitry.myadzelets@gmail.com)}
\address[Second]{CASY, DEI, University of Bologna (e-mail:
andrea.paoli@unibo.it)}
...
Distributed approach does not require to built an entire model of
the system. The approach implies that the system has a set of observation spots,
carrying diagnosability information (diagnosers), each observing only a part of
the system (a subset of the modules, composed). Diagnosers can communicate to
each other in order to decide if a fault occurred. In general, the bigger the observation spots for the same
...
while the system's modules are composed together (we imply that each module is represented
by automata, and the size of a module is reflected by the number of states of
a correspondent automaton).
...
% \begin{alg} Computing the co-faulty and co-non-faulty languages of an
% adjacent module.
...
diagnosable with respect to this new modularity. 
% Since the number of partitionsis equal to the Bell number, which has a double exponential generating function,
Enumerating all the partitions is an intractable problem. Our approach, instead,
uses two recursive procedures which have, in the worst case, exponential time
complexity. As a trade-off the procedures have low space complexity.  
% , and linear state complexity with respect to the number of modules.
We also suggested criteria how to maximize the modularity of the system.
Viability of the approach will be investigated in the further research.  
