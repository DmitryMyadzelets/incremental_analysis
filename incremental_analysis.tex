...
$$
	(\forall s \in L_{j,cf})
	\left[
		P_j^{-1}(s) \subseteq P_i^{-1}(L_{i,f}) \backslash P_i^{-1}(L_{i,nf})
	\right],
$$
and a sublanguage $L_{j,cnf} \subseteq L_j$ is
\emph{co-non-faulty} with respect to $L_{i,nf}$ if 
$$
	(\forall s \in L_{j,cnf})
	\left[
		P_j^{-1}(s) \subseteq P_i^{-1}(L_{i,nf}) \backslash P_i^{-1}(L_{i,f})
	\right].
$$
\end{definition}
...
Similarly, the co-non-faulty language is a sublanguage which satisfies
conditions:
...
co-faulty language $L_{i,cf}$ as follows. Firstly, apply a corresponded
projection for the global sublanguages, and then subtract from the results their
intersection:
\begin{equation}
...
\begin{algorithmic}[1]
	...
	\State $F \leftarrow P_{\Sigma_i \rightarrow \Sigma_c}(L_{i,f})$
	\State $N \leftarrow P_{\Sigma_i \rightarrow \Sigma_c}(L_{i,nf})$
	\State $L_{j,cf} \leftarrow 
		(P_{\Sigma_j \rightarrow \Sigma_c}^{-1}(F\backslash N)) \cap L_j$
	\State $L_{j,cnf} \leftarrow 
		(P_{\Sigma_j \rightarrow \Sigma_c}^{-1}(N\backslash F)) \cap L_j$
	\\
...
% Given $L_i, L_j \mid i,j \in I, i \neq j$ where $L_i$ is partitioned into
% $L_{i,cf}, L_{i,cnf}$.

Note, that the above algorithm returns the co-faulty sublanguages of the module
$j$ which is co-faulty with respect to the module $i$. However, this
sublanguage, in general, is not equal to the one computed from the global faulty
sublanguage, as in \ref{eq:co-faulty_from_global}. [since there may be many
paths from $i$ to $j$ in a correspondent adjacency graph]
