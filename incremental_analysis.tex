...
co-faulty sublanguage $L_{i=j,cf}$ as follows. First, apply a corresponded
...
\label{eq:co-faulty_from_locals}
	...
	i}^I P_k^{-1}(L_k),
\end{equation}
then, by substituting the global language in \ref{eq:co-faulty_from_global} 
by \ref{eq:co-faulty_from_locals}, and applying properties of natural
projection, the co-faulty sublanguage of the module can be computed as follows:
\begin{equation}
	\begin{array}{ll}
		L_{j,cf} := 
		&
		\bigcap_k^I P_j[P_k^{-1}(L_k) \cap P_i^{-1}(L_{i,f})] 
		\backslash \\
		&
		\bigcap_k^I P_j[P_k^{-1}(L_k) \cap P_i^{-1}(L_{i,nf})],
		\\ & \textrm{where } i\neq j\neq k \in I.
	\end{array}
\end{equation}

[In the above equation we should compose co-faulty languages! As below:]
\begin{equation}
	\begin{array}{ll}
		L_{j,cf} := 
		&
		\bigcap_k^I P_j[P_k^{-1}(L_k) \cap P_i^{-1}(L_{i,cf})] 
		\backslash \\
		&
		\bigcap_k^I P_j[P_k^{-1}(L_k) \cap P_i^{-1}(L_{i,cnf})],
		\\ & \textrm{where } i\neq j\neq k \in I.
	\end{array}
\end{equation}


Below we present the algorithm which, given a module with the co-faulty
